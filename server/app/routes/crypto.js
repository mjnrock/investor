...
import { INDICATOR_OUTPUT_COLUMNS } from "../plugins/technical-analysis/lib/TAHelper.js";
...
export const router = async (__dirname) => {
	...
	cryptoRouter.get("/:symbol", async (req, res) => {
		...
		let filePath = path.join(__dirname, "./app/data/cryptos", fileName);

		if(indicatorName) {
			fileName = `${ symbol }.${ indicatorName }.json`;
			filePath = path.join(__dirname, "./app/data/cryptos", fileName);
		}
